...
if __name__=='__main__':
    ...
    train_loader, val_loader, test_loader, feature_size_dict = load(
        '../datasets/loaders_CD.pkl', 4)

    model = DIN(feature_size_dict=feature_size_dict).to(device=device)
    maxlen = 2
    ...
    for iter in tqdm(range(conf_din['runtime']['epoch'])):
        ...
        for data, mask in train_loader:
            ...
            output = model(context_unixReviewTime, context_itempos, behaviour_itemId, behaviour_itemCat, candidate_itemId, candidate_itemCat, mask)#, mask)
            # print(yhat.size())
            # print(yhat)
            #print(output[:10])
